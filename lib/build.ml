...
type root =
  {name: string; build_cmd: string option; run_cmd: string option; deps: string list}
...
let root_of_chunk (name : string) (chunk : Symbol_table.chunk_def) =
  let root_defs =
    List.filter (fun (d : Symbol_table.definition) -> d.is_root) chunk.definitions
  in
  if root_defs = [] then None
    ...
    let build_cmd =
      List.fold_left
        (fun acc (d : Symbol_table.definition) ->
          match find_opt "build" d.options with
          | Some x ->
              Some x
          | None ->
              acc)
    ...
    let run_cmd =
      List.fold_left
        (fun acc (d : Symbol_table.definition) ->
          match find_opt "run" d.options with
          | Some x ->
              Some x
          | None ->
              acc)
...
let is_ok = function Succeeded -> true | Failed _ | Skipped _ -> false
...
let run ?(dry_run = false) (symbols : Symbol_table.build_result) =
...
let has_failures (r : result) =
  String_map.exists
    (fun _ -> function Failed _ -> true | Succeeded | Skipped _ -> false)
    r.statuses
